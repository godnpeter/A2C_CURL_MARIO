...
class A2C:
    ...
    def __init__(self, env, env_name,  n_envs,storage,gamma, num_actions, device, writer, update_interval, network, target_momentum):
        ...
        self.projection = self.build_projection_layer()
    ...
    def predict(self, obs):
        ...
        action, _, value, _ = self.policy_net(obs)
    ...
    def build_projection_layer(self):
        projection = nn.Sequential(
            nn.Linear(self.policy_net.embedder.output_feature_size, self.policy_net.embedder.output_feature_size)
        )
        projection.cuda()
        return projection
    ...
    def optimize(self, timestep):
        ...
        for batch in batch_generator:
            ...
            current_state_batch, action_batch, reward_batch, next_state_batch, done_batch, return_batch, advantage_batch, data_aug1_batch, data_aug2_batch = batch
            action_pred_batch, probs_sampler_batch, value_pred_batch, _ = self.policy_net(current_state_batch)
            # Value loss
            ...
            _, _, _, data_aug1_embedding_batch = self.policy_net(data_aug1_batch)
            _, _, _, data_aug2_embedding_batch = self.target_policy(data_aug2_batch)
            data_aug2_embedding_batch = data_aug2_embedding_batch.detach()


            proj_data_aug2_embedding_batch = self.projection(data_aug2_embedding_batch).T
            contrastive_logits = torch.matmul(data_aug1_embedding_batch, proj_data_aug2_embedding_batch)
            # when doing torch.max, the result has two keys : value, indices
            # to call the value, we index 0 and to add an additional dimension to the tensor, add None
            contrastive_logits -= torch.max(contrastive_logits, dim=1)[0][:, None]
            labels = torch.arange(batch[0].shape[0]).cuda()
            ssl_loss = F.cross_entropy(contrastive_logits, labels)
            ...
            total_loss = policy_loss + 0.5 * value_loss - 0.01 * entropy_loss + 0.1 * ssl_loss
            ...
            # CURL USES MOMENTUM ENCODER AS TARGET_NET
            self.update_momentum_encoder(self.policy_net, self.target_policy, self.target_momentum)


    def update_momentum_encoder(self, policy_network, target_network, momentum=0.999):
        for param_online, param_target in zip(policy_network.parameters(), target_network.parameters()):
            param_target.data = momentum * param_target.data + (1 - momentum) * param_online.detach().data
    ...
    def evaluate(self):
        ...
        """
        self.policy_net.load_state_dict(torch.load('SuperMarioBros-v0_CURL'+ '-best_model_A2C12-12-2020_19-56-12'))
        mean_reward_list = []
        tot_reward = 0
        current_state = self.env.reset()
